...
//  - From there, you can graft those into CascadeStudio/libs/opencascade.js/dist (following its existing conventions)
...
function Cylinder(radius, height, centered) {
  let curCylinder = CacheOp(arguments, () => {
    let cylinderPlane = new oc.gp_Ax2(
      new oc.gp_Pnt(0, 0, centered ? -height / 2 : 0),
      new oc.gp_Dir(0, 0, 1),
    );
...
function Polygon(points, wire) {
  let curPolygon = CacheOp(arguments, () => {
    ...
    let seg2 = new oc.GC_MakeSegment(
...
function Circle(radius, wire) {
  let curCircle = CacheOp(arguments, () => {
    let circle = new oc.GC_MakeCircle(
...
function BSpline(inPoints, closed) {
  let curSpline = CacheOp(arguments, () => {
    let ptList = new oc.TColgp_Array1OfPnt(
      1,
      inPoints.length + (closed ? 1 : 0),
    );
    for (let pIndex = 1; pIndex <= inPoints.length; pIndex++) {
      ptList.SetValue(pIndex, convertToPnt(inPoints[pIndex - 1]));
    }
    if (closed) {
      ptList.SetValue(inPoints.length + 1, ptList.Value(1));
    }
...
function Text3D(text, size, height, fontName) {
  ...
  let curText = CacheOp(arguments, () => {
    ...
    for (let idx = 0; idx < commands.length; idx++) {
      if (commands[idx].type === 'M') {
        ...
        try {
          ...
          console.error(
            "ERROR: OCC encountered malformed characters when constructing faces from this font (likely self-intersections)!  Try using a more robust font like 'Roboto'.",
          );
        }
        ...
        let lineEdge = new oc.BRepBuilderAPI_MakeEdge(
          new oc.Handle_Geom_BezierCurve(quadraticCurve),
        ).Edge();
        currentWire.Add(new oc.BRepBuilderAPI_MakeWire(lineEdge).Wire());

        lastPoint = nextPoint;
      } else if (commands[idx].type === 'C') {
        let controlPoint1 = new oc.gp_Pnt(
          commands[idx].x1,
          commands[idx].y1,
          0,
        );
        let controlPoint2 = new oc.gp_Pnt(
          commands[idx].x2,
          commands[idx].y2,
          0,
        );
        ...
        let lineEdge = new oc.BRepBuilderAPI_MakeEdge(
          new oc.Handle_Geom_BezierCurve(cubicCurve),
        ).Edge();
        currentWire.Add(new oc.BRepBuilderAPI_MakeWire(lineEdge).Wire());
    ...
    if (height === 0) {
      ...
      let textSolid = Rotate(
        [1, 0, 0],
        -90,
        Extrude(textFaces[textFaces.length - 1], [0, 0, height * size]),
      );
...
function ForEachSolid(shape, callback) {
  ...
  for (
    anExplorer.Init(shape, oc.TopAbs_SOLID);
    anExplorer.More();
    anExplorer.Next()
  ) {
...
function GetNumSolidsInCompound(shape) {
  if (!shape || shape.ShapeType() > 1 || shape.IsNull()) {
...
function GetSolidFromCompound(shape, index, keepOriginal) {
  ...
  let sol = CacheOp(arguments, () => {
    ...
    ForEachSolid(shape, (i, s) => {
      if (i === index) {
        innerSolid = new oc.TopoDS_Solid(s);
      }
      solidsFound++;
    });
    if (solidsFound === 0) {
...
function ForEachShell(shape, callback) {
  ...
  for (
    anExplorer.Init(shape, oc.TopAbs_SHELL);
    anExplorer.More();
    anExplorer.Next()
  ) {
...
function ForEachFace(shape, callback) {
  ...
  for (
    anExplorer.Init(shape, oc.TopAbs_FACE);
    anExplorer.More();
    anExplorer.Next()
  ) {
...
function ForEachWire(shape, callback) {
  ...
  for (
    anExplorer.Init(shape, oc.TopAbs_WIRE);
    anExplorer.More();
    anExplorer.Next()
  ) {
...
function GetWire(shape, index, keepOriginal) {
  if (!shape || shape.ShapeType() > 4 || shape.IsNull()) {
    console.error('Not a wire shape!');
    return shape;
  }
  if (!index) {
    index = 0;
  ...
  let wire = CacheOp(arguments, () => {
    ...
    ForEachWire(shape, (i, s) => {
      if (i === index) {
        innerWire = new oc.TopoDS_Wire(s);
      }
      wiresFound++;
    });
    if (wiresFound === 0) {
      console.error('NO WIRES FOUND IN SHAPE!');
      innerWire = shape;
    }
...
function ForEachEdge(shape, callback) {
  ...
  for (
    anExplorer.Init(shape, oc.TopAbs_EDGE);
    anExplorer.More();
    anExplorer.Next()
  ) {
...
function ForEachVertex(shape, callback) {
  ...
  for (
    anExplorer.Init(shape, oc.TopAbs_VERTEX);
    anExplorer.More();
    anExplorer.Next()
  ) {
...
function FilletEdges(shape, radius, edgeList, keepOriginal) {
  let curFillet = CacheOp(arguments, () => {
    ...
    if (foundEdges == 0) {
      console.error(
        'Fillet Edges Not Found!  Make sure you are looking at the object _before_ the Fillet is applied!',
      );
...
function ChamferEdges(shape, distance, edgeList, keepOriginal) {
  let curChamfer = CacheOp(arguments, () => {
    ...
    if (foundEdges == 0) {
      console.error(
        'Chamfer Edges Not Found!  Make sure you are looking at the object _before_ the Chamfer is applied!',
      );
      return new oc.TopoDS_Solid(shape);
    }
...
function Transform(translation, rotation, scale, shapes) {
  ...
  return CacheOp(arguments, () => {
    if (args.length == 4) {
      ...
      postMessage({
        type: 'createTransformHandle',
        payload: {
          translation: translation,
          rotation: rotation,
          scale: scale,
          lineAndColumn: getCallingLocation(),
        },
      });
      // Transform the Object(s)
      return Translate(
        translation,
        Rotate(rotation[0], rotation[1], Scale(scale, shapes)),
      );
    } else {
      // Create the transform gizmo and add it to the scene
      postMessage({
...
function Translate(offset, shapes, keepOriginal) {
  let translated = CacheOp(arguments, () => {
    ...
    transformation.SetTranslation(
      new oc.gp_Vec(offset[0], offset[1], offset[2]),
    );
    ...
    if (!isArrayLike(shapes)) {
      ...
      for (let shapeIndex = 0; shapeIndex < shapes.length; shapeIndex++) {
        newTrans.push(
          new oc.TopoDS_Shape(shapes[shapeIndex].Moved(translation)),
        );
      }
      return newTrans;
    }
  ...
  if (!keepOriginal) {
    sceneShapes = Remove(sceneShapes, shapes);
  }
...
function Mirror(vector, shapes, keepOriginal) {
  ...
  const mirrored = CacheOp(arguments, () => {
    ...
    mirrorTransform.SetMirror(
      new oc.gp_Ax2(mirrorPlaneOrigin, mirrorPlaneNormal),
    );
    ...
    if (!isArrayLike(shapes)) {
      ...
      for (let shapeIndex = 0; shapeIndex < shapes.length; shapeIndex++) {
        newMirroring.push(
          new oc.BRepBuilderAPI_Transform(shapes, mirrorTransform).Shape(),
        );
      }
      return newMirroring;
    }
...
function Union(objectsToJoin, keepObjects, fuzzValue, keepEdges) {
  ...
  let curUnion = CacheOp(arguments, () => {
    ...
    if (objectsToJoin.length > 1) {
      for (let i = 0; i < objectsToJoin.length; i++) {
        if (i > 0) {
          let combinedFuse = new oc.BRepAlgoAPI_Fuse(
            combined,
            objectsToJoin[i],
          );
    ...
    if (!keepEdges) {
      let fusor = new oc.ShapeUpgrade_UnifySameDomain(combined);
      fusor.Build();
  ...
  for (let i = 0; i < objectsToJoin.length; i++) {
    if (!keepObjects) {
      sceneShapes = Remove(sceneShapes, objectsToJoin[i]);
    }
  }
...
function Difference(
  ...
  let curDifference = CacheOp(arguments, () => {
    if (!mainBody || mainBody.IsNull()) {
      console.error('Main Shape in Difference is null!');
    }
    ...
    if (objectsToSubtract.length >= 1) {
      for (let i = 0; i < objectsToSubtract.length; i++) {
        ...
        let differenceCut = new oc.BRepAlgoAPI_Cut(
          difference,
          objectsToSubtract[i],
        );
  ...
  if (Array.isArray(keepObjects)) {
    if (!keepObjects[0]) {
      sceneShapes = Remove(sceneShapes, mainBody);
    }
    for (let i = 0; i < objectsToSubtract.length; i++) {
      if (!keepObjects[1]) {
        sceneShapes = Remove(sceneShapes, objectsToSubtract[i]);
      }
    }
  } else {
    if (!keepObjects) {
...
function Intersection(objectsToIntersect, keepObjects, fuzzValue, keepEdges) {
  ...
  let curIntersection = CacheOp(arguments, () => {
    ...
    if (objectsToIntersect.length > 1) {
      for (let i = 0; i < objectsToIntersect.length; i++) {
        if (i > 0) {
          let intersectedCommon = new oc.BRepAlgoAPI_Common(
            intersected,
            objectsToIntersect[i],
          );
    ...
    if (!keepEdges) {
      let fusor = new oc.ShapeUpgrade_UnifySameDomain(intersected);
      fusor.Build();
  ...
  for (let i = 0; i < objectsToIntersect.length; i++) {
    if (!keepObjects) {
      sceneShapes = Remove(sceneShapes, objectsToIntersect[i]);
    }
  }
...
function Extrude(face, direction, keepFace) {
  let curExtrusion = CacheOp(arguments, () => {
    return new oc.BRepPrimAPI_MakePrism(
      face,
      new oc.gp_Vec(direction[0], direction[1], direction[2]),
    ).Shape();
  });

  if (!keepFace) {
    sceneShapes = Remove(sceneShapes, face);
  }
...
function Offset(shape, offsetDistance, tolerance, keepShape) {
  if (!shape || shape.IsNull()) {
    console.error('Offset received Null Shape!');
  }
  if (!tolerance) {
    tolerance = 0.1;
  }
...
function RotatedExtrude(wire, height, rotation, keepWire) {
  if (!wire || wire.IsNull()) {
    console.error('RotatedExtrude received Null Wire!');
  }
  let curExtrusion = CacheOp(arguments, () => {
    let upperPolygon = Rotate(
      [0, 0, 1],
      rotation,
      Translate([0, 0, height], wire, true),
    );
...
function Sketch(startingPoint) {
  ...
  this.applyFillets = function () {
    ...
    if (this.fillets.length > 0) {
      ...
      for (let f = 0; f < this.fillets.length; f++) {
        this.fillets[f].disabled = false;
      }

      // Create Fillet Maker 2D
      let makeFillet = new oc.BRepFilletAPI_MakeFillet2d(
      ...
      if (successes > 0) {
        this.faces[this.faces.length - 1] = makeFillet.Shape();
      } else {
        console.log("Couldn't find any of the vertices to fillet!!");
      }
  ...
  this.LineTo = function (nextPoint) {
    ...
    if (nextPoint.X) {
      if (
        this.lastPoint.X() === nextPoint.X() &&
        this.lastPoint.Y() === nextPoint.Y()
      ) {
        return this;
      }
      endPoint = nextPoint;
      ...
      if (
        this.lastPoint.X() === nextPoint[0] &&
        this.lastPoint.Y() === nextPoint[1]
      ) {
        return this;
      }
  ...
  this.ArcTo = function (pointOnArc, arcEnd) {
    ...
    let arcCurve = new oc.GC_MakeArcOfCircle(
      this.lastPoint,
      onArc,
      nextPoint,
    ).Value();
  ...
  this.Circle = function (center, radius, reversed) {
    this.argsString += ComputeHash(arguments, true);
    let circle = new oc.GC_MakeCircle(
      new oc.gp_Ax2(convertToPnt(center), new oc.gp_Dir(0, 0, 1)),
      radius,
    ).Value();
    let edge = new oc.BRepBuilderAPI_MakeEdge(circle).Edge();
...
function Slider(
  ...
  if (typeof precision === 'undefined') {
    ...
    console.error('Slider precision must be an integer');
  }
...
function Checkbox(name = 'Toggle', defaultValue = false) {
  ...
  postMessage({
    type: 'addCheckbox',
    payload: { name: name, default: defaultValue },
  });
  return GUIState[name];
}
...
function TextInput(name = 'Text', defaultValue = '', realTime = false) {
  ...
  postMessage({
    type: 'addTextbox',
    payload: { name: name, default: defaultValue, realTime: realTime },
  });
  return GUIState[name];
}
