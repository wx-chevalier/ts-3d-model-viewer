...
function CacheOp(args, cacheMiss) {
  ...
  postMessage({
    type: 'Progress',
    payload: { opNumber: opNumber++, opType: args.callee.name },
...
function Remove(inputArray, objectToRemove) {
  return inputArray.filter(el => {
    return el.hash !== objectToRemove.hash || el.ptr !== objectToRemove.ptr;
  });
}
