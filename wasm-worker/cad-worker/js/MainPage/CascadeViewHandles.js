...
function initializeHandleGizmos(threejsViewport) {
  ...
  messageHandlers['createTransformHandle'] = function (payload) {
    if (payload.lineAndColumn[0] <= 0) {
      console.error(
        'Transform Gizmo not supported in this browser!  Use Chrome or Firefox!',
      );
      return null;
    }
    let handle = new THREE.TransformControls(
    ...
    handle.onChanged = event => {
      ...
      if (this.environment.controls.enabled) {
        ...
        let fullSwapped = code[lineNum].replace(
          /(Transform\(\[(.*?)\]\,\s*\[\[(.*?)\,(.*?)\,(.*?)\]\,(.*?)]\, (.*?)\,)/,
          updateString,
        );
        if (!code[lineNum].includes(updateString)) {
          ...
          if (fullSwapped === code[lineNum]) {
            code[lineNum] = code[lineNum].replace(
              /(Transform\()/g,
              updateString + ' ',
    ...
    emptyObject.setRotationFromAxisAngle(
      new THREE.Vector3(
        ...
        payload.rotation[0][2],
        -payload.rotation[0][1],
      ),
