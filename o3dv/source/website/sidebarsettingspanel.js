...
class EnvironmentMapPopup extends PopupDialog {
  ...
  ShowPopup(buttonDiv, settings, callbacks) {
    ...
    for (let envMapImage of envMapImages) {
      envMapImage.element = AddDomElement(
        contentDiv,
        "img",
        "ov_environment_map_preview"
      );
      envMapImage.element.setAttribute(
        "src",
        "assets/envmaps/" + envMapImage.name + ".jpg"
      );
    ...
    let backgroundIsEnvMapCheckbox = AddCheckbox(
      contentDiv,
      "use_as_background",
      "Use as background",
...
class SettingsModelDisplaySection extends SettingsSection {
  ...
  Init(settings, callbacks) {
    if (FeatureSet.EnvironmentMap) {
      ...
      this.environmentMapButton.addEventListener("click", () => {
        ...
        this.environmentMapPopup.ShowPopup(
          this.environmentMapButton,
          settings,
          {
    ...
    this.backgroundColorPicker = AddColorPicker(
    ...
    this.edgeDisplayToggle = AddToggle(
      edgeParameterDiv,
      "ov_sidebar_parameter_toggle"
    );
    AddDiv(edgeParameterDiv, "ov_sidebar_parameter_text", "Show Edges");

    this.edgeSettingsDiv = AddDiv(
      this.contentDiv,
      "ov_sidebar_settings_padded"
    );
    ...
    this.edgeColorPicker = AddColorPicker(
      edgeColorInput,
      settings.edgeColor,
      predefinedEdgeColors,
      (color) => {
  ...
  Update(settings) {
    if (this.backgroundColorPicker !== null) {
      this.backgroundColorPicker.setColor(
        "#" + ColorToHexString(settings.backgroundColor)
      );
    }
...
class SettingsImportParametersSection extends SettingsSection {
  ...
  Init(settings, callbacks) {
    ...
    this.defaultColorPicker = AddColorPicker(
      defaultColorInput,
      settings.defaultColor,
      predefinedDefaultColors,
      (color) => {
  ...
  Update(settings) {
    if (this.defaultColorPicker !== null) {
      this.defaultColorPicker.setColor(
        "#" + ColorToHexString(settings.defaultColor)
      );
    }
  }
...
class SettingsAppearanceSection extends SettingsSection {
  ...
  Init(settings, callbacks) {
    ...
    this.darkModeToggle = AddToggle(
      darkModeParameterDiv,
      "ov_sidebar_parameter_toggle"
    );
    this.darkModeToggle.OnChange(() => {
      settings.themeId = this.darkModeToggle.GetStatus()
        ? Theme.Dark
        : Theme.Light;
...
export class SidebarSettingsPanel extends SidebarPanel {
  constructor(parentDiv, settings) {
    ...
    this.sectionsDiv = AddDiv(
      this.contentDiv,
      "ov_sidebar_settings_sections ov_thin_scrollbar"
    );
    this.modelDisplaySection = new SettingsModelDisplaySection(
      this.sectionsDiv
    );
    this.importParametersSection = new SettingsImportParametersSection(
      this.sectionsDiv
    );
    this.appearanceSection = new SettingsAppearanceSection(this.sectionsDiv);

    this.resetToDefaultsButton = AddDiv(
      this.contentDiv,
      "ov_button ov_panel_button outline",
      "Reset to Default"
    );
