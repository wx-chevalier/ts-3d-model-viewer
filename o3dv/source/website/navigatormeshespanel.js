...
import {
  CalculatePopupPositionToElementBottomRight,
  ShowListPopup,
} from "./dialogs.js";
  ...
  OnButtonClick() {
    ...
    this.popup = ShowListPopup(materialItems, {
      calculatePosition: (contentDiv) => {
        return CalculatePopupPositionToElementBottomRight(
          this.button,
          contentDiv
        );
      },
...
export class NavigatorMeshesPanel extends NavigatorPanel {
  constructor(parentDiv) {
    ...
    this.titleButtonsDiv = AddDiv(
      this.titleDiv,
      "ov_navigator_tree_title_buttons"
    );
  ...
  Resize() {
    ...
    SetDomElementHeight(
      this.treeDiv,
      height - titleHeight - buttonsHeight - popupHeight
    );
  }
  ...
  FillButtons(importResult) {
    ...
    if (this.mode === MeshesPanelMode.Simple) {
      CreateButton(
        this.titleButtonsDiv,
        this.buttons.showHideMeshes,
        "right",
        () => {
    ...
    function CreateDummyRootItem(panel, node) {
      ...
      let rootItem = new NodeItem(null, nodeId, {
        onVisibilityChanged: (isVisible) => {
          if (isVisible) {
            SetSvgIconImageElement(
              panel.buttons.showHideMeshes.iconDiv,
              "visible"
            );
          } else {
            SetSvgIconImageElement(
  ...
  IsMeshIsolated(meshInstanceId) {
    ...
    this.EnumerateMeshItems((meshItem) => {
      if (
        !meshItem.GetMeshInstanceId().IsEqual(meshInstanceId) &&
        meshItem.IsVisible()
      ) {
        isIsolated = false;
        return false;
