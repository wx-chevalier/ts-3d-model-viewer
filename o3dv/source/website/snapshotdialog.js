...
export function ShowSnapshotDialog(viewer) {
  ...
  function GetSize(sizes, selectedIndex) {
    ...
    if (selectedSize.size !== null) {
      ...
      return [selectedSize.widthInput.value, selectedSize.heightInput.value];
    }
  }
  ...
  let lastSnapshotSizeName = CookieGetStringVal(
    "ov_last_snapshot_size",
    sizes[1].name
  );
  ...
  customSize.widthInput = AddWidthHeightNumberInput(
    optionsDiv,
    "Width",
    (val) => {
  ...
  customSize.heightInput = AddWidthHeightNumberInput(
    optionsDiv,
    "Height",
    (val) => {
      UpdatePreview(viewer, previewImage, GetSize(sizes, selectedIndex));
  ...
  customSize.widthInput.value = CookieGetIntVal(
    "ov_snapshot_custom_width",
    1000
  );
  customSize.heightInput.value = CookieGetIntVal(
