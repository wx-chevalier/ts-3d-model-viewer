...
export function ConvertMeshToMeshBuffer(mesh) {
  function AddVertexToPrimitiveBuffer(
    mesh,
    indices,
    primitiveBuffer,
    meshVertexToPrimitiveVertices
  ) {
    ...
    function FindMatchingPrimitiveVertex(mesh, primitiveVertices, indices) {
      ...
      for (let i = 0; i < primitiveVertices.length; i++) {
        ...
        let equalColor = IsEqualColor(
          mesh,
          indices.color,
          primitiveVertex.color
        );
        let equalNormal = IsEqualNormal(
          mesh,
          indices.normal,
          primitiveVertex.normal
        );
    ...
    if (meshVertexToPrimitiveVertices.has(indices.vertex)) {
      ...
      let existingPrimitiveVertex = FindMatchingPrimitiveVertex(
        mesh,
        primitiveVertices,
        indices
      );
