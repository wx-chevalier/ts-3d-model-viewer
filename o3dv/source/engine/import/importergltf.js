  ...
  EnumerateData(onData) {
    if (this.sparseReader === null) {
      ...
      for (let i = 0; i < this.dataCount; i++) {
        ...
        if (
          sparseIndex < sparseData.length &&
          sparseData[sparseIndex].index === i
        ) {
...
class GltfExtensions {
  ...
  LoadLibraries(extensionsRequired, callbacks) {
    ...
    if (
      this.draco === null &&
      extensionsRequired.indexOf("KHR_draco_mesh_compression") !== -1
    ) {
      LoadExternalLibrary("loaders/draco_decoder.js")
        .then(() => {
  ...
  ProcessPrimitive(importer, gltf, primitive, mesh) {
    function EnumerateComponents(
      ...
      let attributeArray = new Float32Array(
    ...
    let compressedReader =
      importer.GetReaderFromBufferView(compressedBufferView);
    let compressedArrayBuffer = compressedReader.ReadArrayBuffer(
      compressedBufferView.byteLength
    );
    decoderBuffer.Init(
      new Int8Array(compressedArrayBuffer),
    ...
    EnumerateComponents(
      ...
      decoder,
      dracoMesh,
      extensionParams.attributes.POSITION,
      (vertex) => {
    ...
    if (hasNormals) {
      EnumerateComponents(
        ...
        decoder,
        dracoMesh,
        extensionParams.attributes.NORMAL,
        (normal) => {
    ...
    if (hasUVs) {
      EnumerateComponents(
        ...
        decoder,
        dracoMesh,
        extensionParams.attributes.TEXCOORD_0,
        (uv) => {
          uv.y = -uv.y;
...
export class ImporterGltf extends ImporterBase {
  ...
  ProcessMainFile(gltf, onFinish) {
    let unsupportedExtensions = this.gltfExtensions.GetUnsupportedExtensions(
      gltf.extensionsRequired
    );
    if (unsupportedExtensions.length > 0) {
      this.SetError(
        "Unsupported extension: " + unsupportedExtensions.join(", ") + "."
      );
      onFinish();
      return;
    }
  ...
  ImportModelProperties(gltf) {
    function ImportProperties(model, propertyGroupName, propertyObject) {
      ...
      for (let propertyName in propertyObject) {
        if (
          Object.prototype.hasOwnProperty.call(propertyObject, propertyName)
        ) {
  ...
  ImportMaterial(gltf, gltfMaterial) {
    ...
    if (gltfMaterial.pbrMetallicRoughness !== undefined) {
      ...
      material.diffuseMap = this.ImportTexture(
        gltf,
        gltfMaterial.pbrMetallicRoughness.baseColorTexture
      );
      material.metalnessMap = this.ImportTexture(
        gltf,
    ...
    let newMaterial = this.gltfExtensions.ProcessMaterial(
      gltfMaterial,
      material,
      (textureRef) => {
      ...
      if (gltfImage.uri !== undefined) {
        ...
        if (reader !== null) {
          ...
          textureParams.name =
            ...
            textureIndexString +
            "." +
            GetFileExtensionFromMimeType(gltfImage.mimeType);
  ...
  ImportPrimitive(gltf, primitive, mesh) {
    ...
    if (
      ...
      mode !== GltfRenderMode.TRIANGLE_FAN
    ) {
      return;
    }
  ...
  GetReaderFromAccessor(gltf, accessor) {
    ...
    if (accessor.sparse !== undefined) {
      let indexReader = this.GetReaderFromSparseAccessor(
        gltf,
      ...
      let valueReader = this.GetReaderFromSparseAccessor(
        gltf,
