...
export let ParameterConverter = {
  ...
  CameraToString: function (camera) {
    ...
    let cameraParameters = [
      this.NumberToString(camera.eye.x),
      this.NumberToString(camera.eye.y),
      this.NumberToString(camera.eye.z),
      this.NumberToString(camera.center.x),
      this.NumberToString(camera.center.y),
      this.NumberToString(camera.center.z),
      this.NumberToString(camera.up.x),
      this.NumberToString(camera.up.y),
      this.NumberToString(camera.up.z),
    ].join(",");
  ...
  StringToCamera: function (str) {
    ...
    let camera = new Camera(
      new Coord3D(
        this.StringToNumber(paramParts[0]),
        this.StringToNumber(paramParts[1]),
        this.StringToNumber(paramParts[2])
      ),
      new Coord3D(
        this.StringToNumber(paramParts[3]),
        this.StringToNumber(paramParts[4]),
        this.StringToNumber(paramParts[5])
      ),
      new Coord3D(
        this.StringToNumber(paramParts[6]),
        this.StringToNumber(paramParts[7]),
        this.StringToNumber(paramParts[8])
      )
    );
    return camera;
  },
...
export class ParameterListBuilder {
  ...
  AddEnvironmentSettings(envSettings) {
    this.AddUrlPart(
      "envsettings",
      ParameterConverter.EnvironmentSettingsToString(envSettings)
    );
    return this;
  }
  ...
  AddBackgroundColor(background) {
    this.AddUrlPart(
      "backgroundcolor",
      ParameterConverter.ColorToString(background)
    );
    return this;
  }
  ...
  AddEdgeSettings(edgeSettings) {
    this.AddUrlPart(
      "edgesettings",
      ParameterConverter.EdgeSettingsToString(edgeSettings)
    );
    return this;
  }
...
export class ParameterListParser {
  ...
  GetEnvironmentSettings() {
    ...
    return ParameterConverter.StringToEnvironmentSettings(
