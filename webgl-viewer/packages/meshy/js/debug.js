...
Debug.prototype.line = function (v, w, n, lastonly, o, axis) {
  ...
  for (var i = 0; i <= n; i++) {
    ...
    var vert = w
      .clone()
      .multiplyScalar(i / n)
      .add(v.clone().multiplyScalar((n - i) / n));
    vert.z += o;
...
Debug.prototype.points = function (idx, incr) {
  ...
  var debugMaterial = new THREE.PointsMaterial({
    color: color,
    size: 3,
...
Debug.prototype.lines = function (idx, incr) {
  ...
  var debugLineMaterial = new THREE.LineBasicMaterial({
    color: color,
    linewidth: 1,
  });
  var debugLineMesh = new THREE.LineSegments(
    this.debugLineGeo,
    debugLineMaterial,
  );
