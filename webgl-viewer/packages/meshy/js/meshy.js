...
Meshy = function () {
  ...
  if (!Detector.webgl) {
    ...
    webGLWarning.innerHTML =
      "Welp! Your browser doesn't support WebGL. This page will remain blank.";
  ...
  this.printout.log(
    'Meshy is freely available under the MIT license. Thanks for using!',
  );
  this.printout.log('Supported import formats: OBJ, STL.');
  this.printout.log(
    'Controls: LMB (turn), MMB (pan/zoom), RMB (pan), F (center on model), C (center of mass), W (wireframe), B (build volume), G (gizmo)',
  );
...
Meshy.prototype.generateUI = function () {
  ...
  var importSettingsFolder = this.gui.addFolder(
    'Import Settings',
    'Defaults for the imported mesh.',
  );
  importSettingsFolder
    .add(this, 'importUnits', {
      mm: Units.mm,
      cm: Units.cm,
      inches: Units.inches,
    })
  ...
  exportFolder
    .add(this, 'exportSTLascii')
    .name('Export ASCII STL')
    .title('Export as ASCII STL file.');

  var settingsFolder = this.gui.addFolder(
    'Settings',
    'Settings for computation.',
  );

  settingsFolder
    .add(this, 'isLittleEndian')
    .name('Little endian')
    .title('Endianness toggle for imports and exports.');
  settingsFolder
    .add(this, 'vertexPrecision')
    .name('Vertex precision')
    .onChange(this.setVertexPrecision.bind(this))
    .title(
      'Precision p; 10^p is used as a conversion factor between floating-point and fixed-point coordinates.',
    );

  var displayFolder = this.gui.addFolder(
    'Display',
    'Mesh and build volume display settings.',
  );

  displayFolder
    .add(this, 'displayPrecision', 0, 7)
    .step(1)
    .name('Display precision')
    .onChange(this.setDisplayPrecision.bind(this))
    .title(
      'Maximal number of decimal places for displaying floating-point values.',
    );
  displayFolder
    .add(this, 'toggleGizmo')
    .name('Toggle gizmo')
    .title('Toggle transform gizmo visibility (G).');
  displayFolder
    .add(this, 'toggleAxisWidget')
    .name('Toggle axis widget')
    .title('Toggle axis widget visibility.');
  displayFolder
    .add(this, 'toggleWireframe')
    .name('Toggle wireframe')
    .title('Toggle mesh wireframe (W).');
  displayFolder
    .add(this, 'toggleCOM')
    .name('Toggle center of mass')
    .title('Toggle the center of mass indicator (C).');
  displayFolder
    .add(this, 'cameraToModel')
    .name('Camera to model')
  ...
  this.backgroundColorController = displayFolder
    .addColor(this, 'backgroundColor')
    .name('Background color')
    .onChange(this.setBackgroundColor.bind(this))
    .title('Set background color.');
  this.meshColorController = displayFolder
    .addColor(this, 'meshColor')
    .name('Mesh color')
    .onChange(this.setMeshMaterial.bind(this))
    .title('Set mesh color.');
  displayFolder
    .add(this, 'meshRoughness', 0, 1)
    .name('Mesh roughness')
    .onChange(this.setMeshMaterial.bind(this))
    .title('Set mesh roughness.');
  displayFolder
    .add(this, 'meshMetalness', 0, 1)
    .name('Mesh metalness')
    .onChange(this.setMeshMaterial.bind(this))
    .title('Set mesh metalness.');
  this.meshColorController = displayFolder
    .addColor(this, 'wireframeColor')
    .name('Wireframe color')
    .onChange(this.setWireframeMaterial.bind(this))
    .title('Set wireframe color.');
  var buildVolumeFolder = displayFolder.addFolder(
    'Build Volume',
    'Size and visibility settings for the build volume.',
  );
  buildVolumeFolder
    .add(this, 'toggleBuildVolume')
    .name('Toggle volume')
    .title('Toggle build volume visibility.');
  buildVolumeFolder
    .add(this, 'centerOriginOnBuildPlate')
    .name('Center origin')
    .title(
      'Center the origin on the floor of the build volume or place it in the corner.',
    )
    .onChange(this.makeBuildVolume.bind(this));
  this.buildVolumeXController = buildVolumeFolder
    .add(this.buildVolumeSize, 'x', 0)
    .name('Build volume x')
    .title('Build volume size on x in mm.')
    .onChange(this.makeBuildVolume.bind(this));
  this.buildVolumeYController = buildVolumeFolder
    .add(this.buildVolumeSize, 'y', 0)
    .name('Build volume y')
    .title('Build volume size on y in mm.')
    .onChange(this.makeBuildVolume.bind(this));
  this.buildVOlumeZController = buildVolumeFolder
    .add(this.buildVolumeSize, 'z', 0)
    .name('Build volume z')
  ...
  this.editFolder = this.gui.addFolder(
    'Edit',
  ...
  this.measurementFolder = this.gui.addFolder(
    'Measure',
    'Make calculations based on mouse-placed markers.',
  );
  this.measureConvexHull = false;
  this.buildMeasurementFolder();

  var thicknessFolder = this.gui.addFolder(
    'Mesh Thickness',
    'Visualize approximate local mesh thickness.',
  );
  this.thicknessThreshold = 1.0;
  thicknessFolder
    .add(this, 'thicknessThreshold', 0)
    .name('Threshold')
    .title(
      'Thickness threshold: parts of the mesh below this thickness are shown as too thin.',
    );
  thicknessFolder
    .add(this, 'viewThickness')
    .name('View thickness')
    .title(
      'Calculate mesh thickness: parts of the mesh that are too thin are shown in a color interpolated over the [threshold, 0] range.',
    );
  thicknessFolder
    .add(this, 'clearThicknessView')
    .name('Clear thickness view')
  ...
  repairFolder.add(this, 'repair').name('Repair').title('Repair mesh.');

  this.layerHeight = 0.1;
  this.lineWidth = 0.1;
  this.sliceAxis = 'z';
  this.supportSliceFolder = this.gui.addFolder(
    'Supports & Slicing',
  ...
  this.gui.add(this, 'undo').name('Undo').title('Undo the last edit action.');
  this.gui.add(this, 'redo').name('Redo').title('Redo the previous undo.');
  this.gui.add(this, 'delete').name('Delete').title('Delete the mesh.');
  ...
  this.gizmo = new Gizmo(this.camera, this.renderer.domElement, {
    ...
    scaleOrthogonalHandleWidthSegments: this
      .gizmoScaleOrthogonalHandleWidthSegments,
    scaleOrthogonalHandleHeightSegments: this
      .gizmoScaleOrthogonalHandleHeightSegments,

    rotateHandleOuterRadius: this.gizmoRotateHandleOuterRadius,
    rotateOrthogonalHandleOuterRadius: this
      .gizmoRotateOrthogonalHandleOuterRadius,
    ...
    translateOrthogonalHandleThickness: this
      .gizmoTranslateOrthogonalHandleThickness,
    ...
    onFinishTransform: function () {
      _this.controls.enable();
    },

    getPosition: function () {
      return _this.position.clone();
    },
    ...
    getRotation: function () {
      return _this.rotation.clone();
    },
    setRotation: function (euler) {
      _this.rotation.copy(euler);
    },
    onRotate: this.onRotate.bind(this),
    onFinishRotate: this.onFinishRotate.bind(this),

    getScale: function () {
      return _this.scale.clone();
    },
    setScale: function (scale) {
      _this.scale.copy(scale);
    },
...
Meshy.prototype.exportOBJ = function () {
  this.export('obj');
};
Meshy.prototype.exportSTL = function () {
  this.export('stl');
};
Meshy.prototype.exportSTLascii = function () {
  this.export('stlascii');
};
...
Meshy.prototype.makeRotateTransform = function (invertible) {
  var transform = new Transform('rotate', this.model.getRotation()),
    _this = this;
...
Meshy.prototype.makeScaleTransform = function (invertible) {
  var transform = new Transform('scale', this.model.getScale()),
    _this = this;
...
Meshy.prototype.makeMirrorTransform = function (invertible) {
  var transform = new Transform('mirror'),
    _this = this;

  transform.onApply = function (axis) {
    _this.model.mirror(axis);
  };
  transform.invertible = invertible;
...
Meshy.prototype.makeFlipNormalsTransform = function (invertible) {
  var transform = new Transform('flipNormals'),
    _this = this;

  transform.onApply = function () {
    _this.model.flipNormals();
  };
  transform.invertible = invertible;
...
Meshy.prototype.onTranslate = function () {
  if (!this.currentTransform)
    this.currentTransform = this.makeTranslateTransform();
...
Meshy.prototype.onRotate = function () {
  if (!this.currentTransform)
    this.currentTransform = this.makeRotateTransform();
...
Meshy.prototype.onScaleToSize = function () {
  ...
  var startScale = this.currentTransform
    ? this.currentTransform.startVal
    : this.scale;
...
Meshy.prototype.mirrorX = function (invertible) {
  this.mirror('x', invertible);
};
Meshy.prototype.mirrorY = function (invertible) {
  this.mirror('y', invertible);
};
Meshy.prototype.mirrorZ = function (invertible) {
  this.mirror('z', invertible);
};
...
Meshy.prototype.floorX = function (invertible) {
  this.floor('x', invertible);
};
Meshy.prototype.floorY = function (invertible) {
  this.floor('y', invertible);
};
Meshy.prototype.floorZ = function (invertible) {
  this.floor('z', invertible);
};
Meshy.prototype.floor = function (axis, invertible) {
  ...
  this.position[axis] =
    this.buildVolumeMin[axis] +
    this.model.getPosition()[axis] -
    this.model.getMin()[axis];
...
Meshy.prototype.centerX = function (invertible) {
  this.center('x', invertible);
};
Meshy.prototype.centerY = function (invertible) {
  this.center('y', invertible);
};
Meshy.prototype.centerAll = function (invertible) {
  this.center('all', invertible);
};
...
Meshy.prototype.buildEditFolder = function () {
  ...
  this.editFolder
    .add(this, 'snapTransformationsToFloor')
    .name('Snap to floor')
  ...
  this.setBaseOn = false;
  this.setBaseController = this.editFolder
    .add(this, 'toggleSetBase')
    .name('Set base')
  ...
  var translateFolder = this.editFolder.addFolder(
    'Translate',
    'Translate the mesh on a given axis.',
  );
  this.positionXController = translateFolder
    .add(this.position, 'x')
    .onChange(this.onTranslate.bind(this))
    .onFinishChange(this.onFinishTranslate.bind(this))
    .precision(4);
  this.positionYController = translateFolder
    .add(this.position, 'y')
    .onChange(this.onTranslate.bind(this))
    .onFinishChange(this.onFinishTranslate.bind(this))
    .precision(4);
  this.positionZController = translateFolder
    .add(this.position, 'z')
    .onChange(this.onTranslate.bind(this))
    .onFinishChange(this.onFinishTranslate.bind(this))
    .precision(4);
  ...
  var rotateFolder = this.editFolder.addFolder(
    'Rotate',
  ...
  this.rotationXController = rotateFolder
    .add(this.rotationDeg, 'x', 0, 360)
    .onChange(this.onChangeRotationDegrees.bind(this))
    .onFinishChange(this.onFinishRotate.bind(this));
  this.rotationYController = rotateFolder
    .add(this.rotationDeg, 'y', 0, 360)
    .onChange(this.onChangeRotationDegrees.bind(this))
    .onFinishChange(this.onFinishRotate.bind(this));
  this.rotationZController = rotateFolder
    .add(this.rotationDeg, 'z', 0, 360)
    .onChange(this.onChangeRotationDegrees.bind(this))
    .onFinishChange(this.onFinishRotate.bind(this));

  var scaleFolder = this.editFolder.addFolder(
    'Scale',
    'Scale the mesh by given criteria.',
  );

  var scaleByFactorFolder = scaleFolder.addFolder(
    'Scale by Factor',
    'Scale the mesh by a given factor ',
  );
  this.scaleXController = scaleByFactorFolder
    .add(this.scale, 'x', 0)
    .onChange(this.onScaleByFactor.bind(this))
    .onFinishChange(this.onFinishScale.bind(this));
  this.scaleYController = scaleByFactorFolder
    .add(this.scale, 'y', 0)
    .onChange(this.onScaleByFactor.bind(this))
    .onFinishChange(this.onFinishScale.bind(this));
  this.scaleZController = scaleByFactorFolder
    .add(this.scale, 'z', 0)
    .onChange(this.onScaleByFactor.bind(this))
    .onFinishChange(this.onFinishScale.bind(this));

  var scaleToSizeFolder = scaleFolder.addFolder(
    'Scale to Size',
    'Scale the mesh uniformly to a given size.',
  );

  this.scaleToSizeXController = scaleToSizeFolder
    .add(this.size, 'x', 0)
    .name('x size')
    .onChange(this.onScaleToSize.bind(this))
    .onFinishChange(this.onFinishScale.bind(this));
  this.scaleToSizeYController = scaleToSizeFolder
    .add(this.size, 'y', 0)
    .name('y size')
    .onChange(this.onScaleToSize.bind(this))
    .onFinishChange(this.onFinishScale.bind(this));
  this.scaleToSizeZController = scaleToSizeFolder
    .add(this.size, 'z', 0)
    .name('z size')
    .onChange(this.onScaleToSize.bind(this))
    .onFinishChange(this.onFinishScale.bind(this));

  this.scaleToMeasurementFolder = scaleFolder.addFolder(
  ...
  var ringSizeFolder = scaleFolder.addFolder(
    'Scale To Ring Size',
  ...
  ringSizeFolder
    .add(this, 'measureCircle')
    .name('1. Mark circle')
    .title(
      'Turn on the circle measurement tool and mark the inner circumference of the ring.',
    );
  this.ringSize = 0;
  ringSizeFolder
    .add(this, 'ringSize', ringSizes)
    .name('2. Ring size')
    .title('Select ring size.');
  ringSizeFolder
    .add(this, 'scaleToRingSize')
    .name('3. Scale to size')
    .title('Scale the ring.');
  ringSizeFolder
    .add(this, 'removeCurrentMeasurement')
    .name('4. End measurement')
    .title('Turn off the measurement tool (ESC).');

  var mirrorFolder = this.editFolder.addFolder(
    'Mirror',
    'Mirror the mesh on a given axis in object space.',
  );
  mirrorFolder
    .add(this, 'mirrorX')
    .name('Mirror on x')
  ...
  var floorFolder = this.editFolder.addFolder(
    'Floor',
    'Floor the mesh on a given axis.',
  );
  floorFolder
    .add(this, 'floorX')
    .name('Floor to x')
  ...
  var centerFolder = this.editFolder.addFolder(
    'Center',
    'Center the mesh on a given axis in the build volume.',
  );
  centerFolder
    .add(this, 'centerAll')
    .name('Center on all')
...
Meshy.prototype.scaleToRingSize = function () {
  ...
  if (
    !measurement ||
    !measurement.active ||
    measurement.getType() !== Measurement.Types.circle ||
    // must have a measurement result and a diameter field in that result
    !result ||
    result.diameter === undefined
  ) {
    this.printout.warn(
      'Scaling to ring size requires an active, valid circle measurement.',
    );
    return;
  }
...
Meshy.prototype.buildMeasurementFolder = function () {
  ...
  this.measurementFolder
    .add(this, 'measureLength')
    .name('Length')
    .title('Measure point-to-point length.');
  this.measurementFolder
    .add(this, 'measureAngle')
    .name('Angle')
    .title(
      'Measure angle (in degrees) between two segments formed by three consecutive points.',
    );
  this.measurementFolder
    .add(this, 'measureCircle')
    .name('Circle')
  ...
  if (this.previewMarkerRadiusOffset === undefined)
    this.previewMarkerRadiusOffset = false;
  this.measurementFolder
    .add(this, 'measureLocalCrossSection')
    .name('Local cross-section')
    .title('Measure the cross-section of a single part of the mesh.');
  this.measurementFolder
    .add(this, 'measureConvexHull')
    .name('Convex hull')
  ...
  this.crossSectionArrayFolder = this.measurementFolder.addFolder(
    'Cross-section array',
    'Make multiple cross-section measurements.',
  );
  this.crossSectionArrayFolder
    .add(this, 'crossSectionArrayAxis', ['x', 'y', 'z'])
    .name('Axis')
    .title('Axis normal to the cross-section planes.');
  this.crossSectionArrayFolder
    .add(this, 'crossSectionArrayOffset', 0)
    .precision(4)
    .name('Min offset')
    .title("Offset from the mesh's minimum bound on the measurement axis.");
  this.crossSectionArrayFolder
    .add(this, 'crossSectionArrayIncrement', 0.0001)
    .precision(4)
    .name('Increment')
    .title('Distance between adjacent cross-section planes.');
  this.crossSectionArrayFolder
    .add(this, 'crossSectionArrayFilename')
    .name('Filename')
    .title('Filename for the measurement result.');
  this.crossSectionArrayFolder
    .add(this, 'crossSectionArrayFormat', ['json', 'csv'])
    .name('Format')
    .title('Output format for the measurement result.');
  this.crossSectionArrayFolder
    .add(this, 'measureCrossSectionArray')
    .name('Measure')
    .title(
      'Measure an array of cross-sections and export a JSON file with the results.',
    );
  ...
  if (this.measurementsExist()) {
    ...
    this.measurementFolder
      .add(this, 'measurementIdx', indices)
      .name('Current')
    ...
    this.measurementFolder
      .add(this, 'removeAllMeasurements')
      .name('Remove all')
...
Meshy.prototype.getCurrentMeasurementResult = function () {
  if (this.measurementIdx >= 0)
    return this.measurementData[this.measurementIdx].result;
  else return null;
};
...
Meshy.prototype.measureCrossSectionArray = function () {
  ...
  if (format === 'json') {
    ...
    var csvContent = results
      .map(function (result) {
        return result.position + ',' + result.area + ',' + result.length + '\n';
      })
      .join('');
  ...
  downloadAnchorNode.setAttribute(
    'download',
    this.crossSectionArrayFilename + '.' + format,
  );
...
Meshy.prototype.addMeasurement = function (params) {
  ...
  params.color = !this.measurementsExist()
    ? 0x8adeff
    : Math.round((Math.random() / 2.0 + 0.5) * 0xffffff);
  ...
  if (type === Measurement.Types.length) {
    item.list = this.infoBox.addList(
      item.measurement.uuid,
      'Length',
      params.color,
    );
...
Meshy.prototype.buildScaleToMeasurementFolder = function () {
  ...
  this.scaleToMeasurementFolder
    .add(this, 'measurementToScale', this.scalableMeasurements)
  ...
  this.measurementToScaleValueController = this.scaleToMeasurementFolder
    .add(this, 'measurementToScaleValue')
    .name('Value')
    .title(
      'Updating this field scales the mesh so that the measurement equals this value.',
    )
...
Meshy.prototype.generateSupports = function () {
  if (this.model) {
    if (this.supportRadius < this.lineWidth) {
      ...
      this.printout.warn(
        'Support taper radius is lower than the planar resolution. This may result in missing support slices.',
      );
    }
...
Meshy.prototype.buildSupportSliceFolder = function () {
  ...
  if (this.sliceModeOn) {
    ...
    supportSliceFolder
      .add(this, 'layerHeight', 0.0001, 1)
      .name('Layer height')
      .title('Height of each mesh slice layer.');
    supportSliceFolder
      .add(this, 'lineWidth', 0.0001, 1)
      .name('Line width')
      .title(
        'Width of the print line. Affects minimum resolvable detail size, decimation of sliced contours, and extrusion in the exported G-code.',
      );
    supportSliceFolder
      .add(this, 'sliceAxis', ['x', 'y', 'z'])
      .name('Up axis')
      .title('Axis normal to the slicing planes.');

    var supportFolder = supportSliceFolder.addFolder(
      'Supports',
      'Generate supports for printing the model.',
    );
...
Meshy.prototype.buildSupportFolder = function (folder) {
  folder
    .add(this, 'supportAngle', 0, 89)
    .name('Angle')
    .title('Angle defining faces that need support.');
  folder
    .add(this, 'supportSpacingFactor', 1, 100)
    .name('Spacing factor')
    .title('Greater spacing factor makes supports more sparse.');
  folder
    .add(this, 'supportRadius', 0.0001, 1)
    .name('Radius')
    .title(
      'Base radius for supports. NB: if this radius is too low in comparison with line width, the supports may not print correctly.',
    );
  folder
    .add(this, 'supportTaperFactor', 0, 1)
    .name('Taper factor')
    .title('Defines how much the supports taper when connected to the mesh.');
  folder
    .add(this, 'supportSubdivs', 4)
    .name('Subdivs')
    .title('Number of subdivisions in the cylindrical support struts.');
  folder
    .add(this, 'supportRadiusFnName', ['constant', 'sqrt'])
    .name('Radius function')
    .title(
      'Function that defines how support radius grows with the volume it supports; default is square root.',
    );
  folder
    .add(this, 'supportRadiusFnK', 0)
    .name('Function constant')
    .title(
      'Multiplicative constant that modifies the support radius function.',
    );
  folder
    .add(this, 'generateSupports')
    .name('Generate supports')
    .title('Generate the supports.');
  folder
    .add(this, 'removeSupports')
    .name('Remove supports')
...
Meshy.prototype.buildSliceDisplayFolder = function (folder) {
  ...
  if (this.sliceMode === Slicer.Modes.preview) {
    folder
      .add(this, 'slicePreviewModeSliceMesh', true)
      .name('Show sliced mesh')
      .onChange(this.updateSlicerDisplayParams.bind(this))
      .title(
        'If checked, the mesh is shown sliced by the current slicing plane; else, the mesh is shown as a ghost.',
      );
  } else if (this.sliceMode === Slicer.Modes.full) {
    folder
      .add(this, 'sliceFullModeUpToLayer')
      .name('Up to layer')
      .onChange(this.updateSlicerDisplayParams.bind(this))
      .title('Display all contours, or all contours up to a given layer.');
    folder
      .add(this, 'sliceFullModeShowInfill')
      .name('Show infill')
      .onChange(this.updateSlicerDisplayParams.bind(this))
      .title(
        'Show infill if checked; default setting is false because infill makes the layers hard to see.',
      );
  }
};
Meshy.prototype.buildSliceFolder = function (folder) {
  ...
  if (this.sliceModeOn) {
    ...
    this.sliceLevelController = folder
      .add(this, 'currentSliceLevel')
      .min(minLevel)
      .max(maxLevel)
      .step(1)
    ...
    folder
      .add(this, 'sliceMode', {
        preview: Slicer.Modes.preview,
        full: Slicer.Modes.full,
      })
      .name('Mode')
      .onChange(this.setSliceMode.bind(this))
      .title(
        'Set slicer mode: preview mode shows the mesh sliced at a particular level; full mode shows all layers simultaneously.',
      );

    this.sliceDisplayFolder = folder.addFolder(
      'Display',
      'Display options for the current slice mode.',
    );
  ...
  if (this.sliceModeOn)
    folder
      .add(this, 'endSliceMode')
      .name('Slice mode off')
      .title('Turn slice mode off.');
  else
    folder
      .add(this, 'startSliceMode')
      .name('Slice mode on')
      .title('Turn slice mode on.');
};

Meshy.prototype.buildLayerSettingsFolder = function (folder) {
  var sliceLayerSettingsFolder = folder.addFolder(
    'Layer Settings',
    'Settings for computing layers.',
  );
  this.clearFolder(sliceLayerSettingsFolder);

  sliceLayerSettingsFolder
    .add(this, 'sliceNumWalls', 1, 10)
    .name('Walls')
    .step(1)
    .title(
      'Number of horizontal walls between the print exterior and the interior.',
    );
  sliceLayerSettingsFolder
    .add(this, 'sliceNumTopLayers', 1, 10)
    .name('Top layers')
    .step(1)
    .title(
      'Number of layers of solid infill that must be present between the print interior and exterior in the vertical direction.',
    );
  sliceLayerSettingsFolder
    .add(this, 'sliceOptimizeTopLayers')
    .name('Optimize top layers')
    .title(
      'Calculate the top layers in an optimized way. This may result in slightly less accurate solid infill computation but should cheapen computation.',
    );
  sliceLayerSettingsFolder
    .add(this, 'sliceInfillType', {
    ...
    .name('Infill type')
    .title(
      "Print infill type: fills the parts of each contour that aren't occupied by solid infill forming top layers. If 'none' is selected, solid top layer infill is still generated.",
    );
  sliceLayerSettingsFolder
    .add(this, 'sliceInfillDensity', 0, 1)
    .name('Infill density')
    .title('0 density means no infill, 1 means solid.');
  sliceLayerSettingsFolder
    .add(this, 'sliceInfillOverlap', 0, 1)
    .name('Infill overlap')
    .title(
      'Defines how much infill overlaps with the innermost wall. 0 gives a separation of a full line width, 1 means the printline of an infill line starts and ends on the centerline of the wall.',
    );
  if (this.sliceModeOn) {
    sliceLayerSettingsFolder
      .add(this, 'updateSlicerParams')
      .name('Apply')
      .title('Update the layer parameters and recalculate as necessary.');
  }
};
Meshy.prototype.buildRaftFolder = function (folder) {
  var sliceRaftFolder = folder.addFolder(
    'Raft',
    'Settings for computing the raft.',
  );
  this.clearFolder(sliceRaftFolder);

  sliceRaftFolder
    .add(this, 'sliceMakeRaft')
    .name('Make raft')
    .title(
      'Checked if the slicer needs to generate a raft. The raft is formed from several layers of infill to provide initial adhesion to the build plate.',
    );
  sliceRaftFolder
    .add(this, 'sliceRaftNumBaseLayers', 0)
    .step(1)
    .name('Base layers')
    .title(
      'Number of raft base layers. These layers are printed slowly to ensure initial adhesion.',
    );
  sliceRaftFolder
    .add(this, 'sliceRaftBaseLayerHeight', 0)
    .name('Base height')
    .title('Print height of the raft base layers.');
  sliceRaftFolder
    .add(this, 'sliceRaftBaseLineWidth', 0)
    .name('Base width')
    .title('Line width of the raft base layers.');
  sliceRaftFolder
    .add(this, 'sliceRaftBaseDensity', 0, 1)
    .name('Base density')
    .title('Density of the infill forming the raft base layers.');
  sliceRaftFolder
    .add(this, 'sliceRaftNumTopLayers', 0)
    .step(1)
    .name('Top layers')
    .title('Number of additional layers on top of the raft base layers.');
  sliceRaftFolder
    .add(this, 'sliceRaftTopLayerHeight', 0)
    .name('Top height')
    .title('Print height of the raft top layers.');
  sliceRaftFolder
    .add(this, 'sliceRaftTopLineWidth', 0)
    .name('Top width')
    .title('Line width of the raft top layers.');
  sliceRaftFolder
    .add(this, 'sliceRaftTopDensity', 0, 1)
    .name('Top density')
    .title('Density of the infill forming the raft top layers.');
  sliceRaftFolder
    .add(this, 'sliceRaftOffset', 0)
    .name('Offset')
    .title(
      'Horizontal outward offset distance of the raft from the bottom of the mesh. A wider raft will adhere to the build plate better.',
    );
  sliceRaftFolder
    .add(this, 'sliceRaftGap', 0)
    .name('Air gap')
    .title(
      'Small air gap between the top of the raft and the bottom of the main print to make detaching the print easier.',
    );
  sliceRaftFolder
    .add(this, 'sliceRaftWriteWalls')
    .name('Print perimeter')
...
Meshy.prototype.buildGcodeFolder = function (folder) {
  var gcodeFolder = folder.addFolder(
    'G-code',
    'Settings for computing the G-code.',
  );
  this.clearFolder(gcodeFolder);

  this.gcodeFilenameController = gcodeFolder
    .add(this, 'gcodeFilename')
    .name('Filename')
    .title('Filename to save.');
  gcodeFolder
    .add(this, 'gcodeExtension', { gcode: 'gcode' })
    .name('Extension')
    .title('File extension.');
  gcodeFolder
    .add(this, 'gcodeTemperature', 0)
    .name('Temperature')
    .title('Extruder temperature.');
  gcodeFolder
    .add(this, 'gcodeFilamentDiameter', 0.1, 5)
    .name('Filament diameter')
    .title(
      'Filament diameter (mm); affects the computation of how much to extrude.',
    );
  gcodeFolder
    .add(this, 'gcodePrimeExtrusion', 0)
    .name('Prime extrusion')
    .title('Small length (mm) of filament to extrude for priming the nozzle.');
  gcodeFolder
    .add(this, 'gcodeExtrusionMultiplier', 0)
    .name('Extrusion multiplier')
    .title(
      'Factor that can be used to tweak under- or over-extrusion. Directly multiplies gcode extrusion values. Default is 1.',
    );
  gcodeFolder
    .add(this, 'gcodeInfillSpeed', 0)
    .name('Infill speed')
    .title(
      'Speed (mm/s) at which infill is printed. Infill is less sensitive to accuracy issues, so it can be printed more quickly than the walls.',
    );
  gcodeFolder
    .add(this, 'gcodeWallSpeed', 0)
    .name('Wall speed')
    .title('Speed (mm/s) at which the walls are printed.');
  gcodeFolder
    .add(this, 'gcodeRaftBasePrintSpeed', 0)
    .name('Raft base speed')
    .title(
      'Speed (mm/s) at which the raft base layer should be printed. Should be slow so that the layer is thick and adheres properly.',
    );
  gcodeFolder
    .add(this, 'gcodeRaftTopPrintSpeed', 0)
    .name('Raft top speed')
    .title('Speed (mm/s) at which the raft top layer should be printed.');
  gcodeFolder
    .add(this, 'gcodeTravelSpeed', 0)
    .name('Travel speed')
    .title('Speed (mm/s) at which the extruder travels while not printing.');
  gcodeFolder
    .add(this, 'gcodeCoordinatePrecision', 0)
    .name('Coord precision')
    .title(
      'Number of digits used for filament position coordinates. More digits increases file size.',
    );
  gcodeFolder
    .add(this, 'gcodeExtruderPrecision', 0)
    .name('Extruder precision')
    .title(
      'Number of digits used for extrusion values. More digits increases file size.',
    );
  if (this.sliceModeOn) {
    gcodeFolder
      .add(this, 'gcodeSave', 0)
      .name('Save G-code')
...
Meshy.prototype.setWireframeMaterial = function () {
  if (this.model)
    this.model.setWireframeMaterialParams({
...
Meshy.prototype.initViewport = function () {
  ...
  function init() {
    ...
    _this.camera = new THREE.PerspectiveCamera(30, width / height, 0.1, 10000);
    ...
    _this.controls = new Controls(_this.camera, _this.container, {
...
Meshy.prototype.calculateBuildVolumeCenter = function () {
  if (!this.buildVolumeMin || !this.buildVolumeMax)
    this.calculateBuildVolumeBounds();
...
Meshy.prototype.makeBuildVolume = function () {
  ...
  this.calculateBuildVolumeBounds();
  var min = this.buildVolumeMin,
    max = this.buildVolumeMax;
...
Meshy.prototype.import = function () {
  if (this.model) {
    this.printout.warn(
      'A model is already loaded; delete the current model to import a new one.',
    );
    return;
  }
...
Meshy.prototype.export = function (format) {
  ...
  try {
    ...
    this.printout.log(
      "Saved file '" + this.filename + "' as " + format.toUpperCase(),
    );
  } catch (e) {
    this.printout.error(e);
  }
