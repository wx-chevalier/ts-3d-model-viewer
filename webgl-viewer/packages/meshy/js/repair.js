    ...
    while (true) {
      ...
      while (true) {
        ...
        if (previous == null) {
          ...
          if (
            currentAdjacentData.windingOrder[
              currentAdjacentData.neighbors.indexOf(next)
            ] < 0
          ) {
            next = neighbors[1];
          }
        ...
        //    that borders the same hole
          ...
          if (data.numHoles == 1) {
            ...
            for (var i = 0; i < neighbors.length; i++) {
              ...
              edges[i] = edges[i]
                .sub(normal.clone().multiplyScalar(normal.dot(edges[i])))
                .normalize();
            }
            ...
            for (var i = 0; i < edges.length; i++) {
              ...
              if (edge.dot(orthogonalVector) < 0)
    ...
    for (var c = 0; c < borderCycles.length; c++) {
      ...
      while (cycle.length > 3) {
        ...
        if (count % originalCycleLength == 0) {
          var newPathLength = edges.reduce(function (acc, x) {
            return acc + x.length();
          }, 0);
        ...
        var patchprevidx = vertexMapIdx(
          patchVertexMap,
          vprev,
          patchVertices,
          p,
        );
        var patchnextidx = vertexMapIdx(
          patchVertexMap,
          vnext,
          patchVertices,
          p,
        ...
        if (angle < 1.308996939) {
          ...
          var prevlen = eprev.length(),
            nextlen = enext.length();
        ...
        if (newVerts.length == 0) {
          ...
          faceAngle =
            Math.acos(
              edges[idx]
                .clone()
                .normalize()
                .dot(v.clone().sub(cycle[idx]).normalize()),
            ) * 2.0;
          normals[idx]
            .multiplyScalar(2 * Math.PI - angle)
            .add(face.normal.clone().multiplyScalar(faceAngle))
            .normalize();
          faceAngle =
            Math.acos(
              edges[idx]
                .clone()
                .normalize()
                .dot(cycle[nextIdx].clone().sub(v).normalize()),
            ) * 2.0;
          normals[nextIdx]
            .multiplyScalar(2 * Math.PI - angles[nextIdx])
            .add(face.normal.clone().multiplyScalar(faceAngle))
            .normalize();
          // recalculate angles
          angles[idx] = calculateAngleFromEdges(idx, edges, cycle, normals, n);
          angles[nextIdx] = calculateAngleFromEdges(
            nextIdx,
            edges,
            cycle,
            normals,
          ...
          faceAngle =
            Math.acos(
              edges[prevIdx]
                .clone()
                .normalize()
                .dot(v.clone().sub(cycle[prevIdx]).normalize()),
            ) * 2.0;
          normals[prevIdx]
            .multiplyScalar(2 * Math.PI - angles[prevIdx])
            .add(face1.normal.clone().multiplyScalar(faceAngle))
            .normalize();
          ...
          normals[nextIdx]
            .multiplyScalar(2 * Math.PI - angles[nextIdx])
            .add(face2.normal.clone().multiplyScalar(faceAngle))
            .normalize();
          // recalculate angles
          angles[prevIdx] = calculateAngleFromEdges(
            prevIdx,
            edges,
            cycle,
            normals,
          ...
          faceAngle =
            Math.acos(
              edges[nextIdx]
                .clone()
                .normalize()
                .dot(cycle[nextnextIdx].clone().sub(v).normalize()),
            ) * 2.0;
          normals[nextnextIdx]
            .multiplyScalar(2 * Math.PI - angles[nextnextIdx])
            .add(face3.normal.clone().multiplyScalar(faceAngle))
            .normalize();
          // recalculate angles
          angles[prevIdx] = calculateAngleFromEdges(
            prevIdx,
          ...
          angles[nextnextIdx] = calculateAngleFromEdges(
            nextnextIdx,
            edges,
            cycle,
            normals,
      ...
      for (var key in patchAdjacencyMap) {
        if (cycle.indexOf(patchAdjacencyMap[key].vertex) > -1) {
          ...
          data.copyNeighbors = data.neighbors.map(function (x) {
            return x.clone();
          });
        }
      }
              ...
              .multiplyScalar(
                Math.acos(
                  vertex1
                    ...
                    .sub(vertex)
                    .normalize()
                    .dot(vertex2.clone().sub(vertex).normalize()),
                ),
              ),
          );
        }
