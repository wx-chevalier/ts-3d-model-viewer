...
var Octree = (function () {
  function Octree(mesh, params) {
    ...
    var size =
      params.size ||
      vector3MaxElement(this.max.clone().sub(this.min)) + overflow;
    var depth;
  ...
  TreeNode.prototype.addFace = function (face, idx) {
    ...
    for (var i = 0; i < 8; i++) {
      ...
      if (cubeIntersectsTri(co, cs, face)) {
        if (child === undefined)
          this.children[i] = new TreeNode(depth - 1, co, cs);
  ...
  TreeNode.prototype.calculateEdgeIntersections = function (faces, vertices) {
    ...
    if (depth == 0) {
      ...
      for (var i = 0; i < 8; i++) {
        var child = this.children[i];
        if (child !== undefined)
          child.calculateEdgeIntersections(faces, vertices);
      }
    }
  };
  ...
  TreeNode.prototype.visualize = function (geo, drawLines, depthLimit) {
    if (
      this.depth == 0 ||
      (depthLimit !== undefined && this.depth == depthLimit)
    ) {
      if (drawLines) {
  ...
  Raycaster.prototype.castRayProc = function (node, t0, t1) {
    ...
    // each child node the ray crosses (in the order it crosses them)
      ...
      if (dir.x == 0) {
        if (p.x < nodeCenter.x) currentChildIdx &= ~1;
        // unset x bit
        else currentChildIdx |= 1;
      }
      if (dir.y == 0) {
        if (p.y < nodeCenter.y) currentChildIdx &= ~2;
        // unset x bit
        else currentChildIdx |= 2;
      }
      if (dir.z == 0) {
        if (p.z < nodeCenter.z) currentChildIdx &= ~4;
        // unset x bit
      ...
      while (currentChildIdx > -1) {
        ...
        if (child) {
          var intersection = this.castRayProc(
            child,
            childParams.t0,
            childParams.t1,
          );
