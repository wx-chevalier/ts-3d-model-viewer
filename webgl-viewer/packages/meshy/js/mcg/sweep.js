    ...
    function sweep(operation, srcA, srcB) {
      ...
      var events = new PriorityQueue({
        comparator: function (a, b) {
          return a.sweepcompare(b);
        },
      });

      // structure storing the sweep line status
      var status = new RBTree(function (a, b) {
        return a.linecompare(b);
      });
      ...
      function handleEventIntersection(a, b) {
        ...
        var ca = coincident(pi, pa),
          cta = coincident(pi, pta);
        var cb = coincident(pi, pb),
          ctb = coincident(pi, ptb);
      ...
      function eventPairComparisonPrint(ep, ee, force) {
        ...
        if (printEvents && ep && ee) {
          ...
          console.log(
            ...
            ep.scompare(ee),
            ee.scompare(ep),
            lc(ep.p, ep.twin.p, ee.twin.p),
            lc(ee.p, ee.twin.p, ep.twin.p),
            lc(ep.p, ep.twin.p, ee.p),
            lc(ee.p, ee.twin.p, ep.p),
            ee.intersects(ep),
            ef.interpolate(es.p.h).h,
            ef.interpolate(es.p.h).v,
          );
        }
      }
      ...
      function statusDraw(ev, factor, d, force) {
        ...
        while ((e = iter.next()) !== null) {
          ...
          debug.line(
            epc.toVector3(THREE.Vector3, ctx),
            etpc.toVector3(THREE.Vector3, ctx),
            1,
            false,
            0,
            ctx.axis,
        ...
        debug.line(
          top.toVector3(THREE.Vector3, ctx),
          bot.toVector3(THREE.Vector3, ctx),
          1,
          false,
          0,
          ctx.axis,
      ...
      function eventDraw(e, offset, color, force) {
        ...
        debug.oneline(
          e.p.toVector3(THREE.Vector3, context),
          e.twin.p.toVector3(THREE.Vector3, context),
          offset,
          axis,
          color,
        );
