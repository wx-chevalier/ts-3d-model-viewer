...
var preloadedFonts = [
  (window.cadWorkerBasePath || '/cad-worker') + '/fonts/Roboto.ttf',
  (window.cadWorkerBasePath || '/cad-worker') + '/fonts/Papyrus.ttf',
  (window.cadWorkerBasePath || '/cad-worker') + '/fonts/Consolas.ttf',
];
...
new opencascade({
  locateFile(path) {
    if (path.endsWith('.wasm')) {
      return (window.cadWorkerBasePath || '/cad-worker') + '/cad-worker.wasm';
    }
    return path;
  },
}).then(openCascade => {
...
function combineAndRenderShapes(payload) {
  ...
  postMessage({
    type: 'Progress',
    payload: { opNumber: opNumber++, opType: 'Combining Shapes' },
  });
  ...
  if (sceneShapes.length > 0) {
    for (let shapeInd = 0; shapeInd < sceneShapes.length; shapeInd++) {
      if (
        ...
        !sceneShapes[shapeInd].IsNull ||
        sceneShapes[shapeInd].IsNull()
      ) {
        console.error(
          'Null Shape detected in sceneShapes; skipping: ' +
            JSON.stringify(sceneShapes[shapeInd]),
        );
        continue;
      }
      ...
      Object.assign(
        fullShapeEdgeHashes,
        ForEachEdge(sceneShapes[shapeInd], (index, edge) => {}),
      );
    ...
    postMessage({
      type: 'Progress',
      payload: { opNumber: opNumber++, opType: 'Triangulating Faces' },
    });
    let facesAndEdges = ShapeToMesh(
      currentShape,
      payload.maxDeviation || 0.1,
      fullShapeEdgeHashes,
      fullShapeFaceHashes,
    );
    sceneShapes = [];
    postMessage({
      type: 'Progress',
      payload: { opNumber: opNumber, opType: '' },
    }); // Finish the progress
