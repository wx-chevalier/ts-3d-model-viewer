...
function importSTEPorIGES(fileName, fileText) {
  ...
  if (readResult === 1) {
    console.log(
      fileName + ' loaded successfully!     Converting to OCC now...',
    );
    ...
    console.log(
      "Shape Import complete! Use sceneShapes.push(externalShapes['" +
        fileName +
...
function importSTL(fileName, fileText) {
  ...
  if (reader.Read(readShape, fileName)) {
    ...
    console.log(
      'Something in OCCT went wrong trying to read ' +
        fileName +
        '.  \n' +
...
messageHandlers['clearExternalFiles'] = () => {
  externalShapes = {};
};
