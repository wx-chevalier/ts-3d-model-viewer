  ...
  try {
    ...
    ForEachFace(shape, (faceIndex, myFace) => {
      ...
      ForEachEdge(myFace, (index, myEdge) => {
        ...
        if (fullShapeEdgeHashes2.hasOwnProperty(edgeHash)) {
          ...
          let myP = oc.BRep_Tool.prototype.PolygonOnTriangulation(
            myEdge,
            myT,
            aLocation,
          );
          ...
          for (let j = 0; j < edgeNodes.Length(); j++) {
            ...
            this_edge.vertex_coord[j * 3 + 0] =
              this_face.vertex_coord[(vertexIndex - 1) * 3 + 0];
            this_edge.vertex_coord[j * 3 + 1] =
              this_face.vertex_coord[(vertexIndex - 1) * 3 + 1];
            this_edge.vertex_coord[j * 3 + 2] =
              this_face.vertex_coord[(vertexIndex - 1) * 3 + 2];
          }
    ...
    ForEachEdge(shape, (index, myEdge) => {
      ...
      if (!fullShapeEdgeHashes2.hasOwnProperty(edgeHash)) {
        ...
        let tangDef = new oc.GCPnts_TangentialDeflection(
          adaptorCurve,
          maxDeviation,
          0.1,
        );
    ...
    setTimeout(() => {
      err.message =
        'INTERNAL OPENCASCADE ERROR DURING GENERATE: ' + err.message;
      throw err;
    }, 0);
  }
