...
export class Importer3dm extends ImporterBase {
  ...
  ImportContent(fileContent, onFinish) {
    if (this.rhino === null) {
      LoadExternalLibrary("loaders/rhino3dm.min.js")
        .then(() => {
          rhino3dm().then((rhino) => {
            this.rhino = rhino;
  ...
  InitRhinoInstances(rhinoDoc) {
    ...
    for (let i = 0; i < rhinoInstanceDefinitions.count(); i++) {
      ...
      this.instanceIdToDefinition.set(
        rhinoInstanceDefinition.id,
        rhinoInstanceDefinition
      );
    }
  }
    ...
    if (docStrings.count() > 0) {
      ...
      for (let i = 0; i < docStrings.count(); i++) {
        ...
        propertyGroup.AddProperty(
          new Property(PropertyType.Text, docString[0], docString[1])
        );
      }
  ...
  ImportRhinoGeometryObject(rhinoDoc, rhinoObject, rhinoInstanceReferences) {
    ...
    if (
      rhinoAttributes.isInstanceDefinitionObject &&
      rhinoInstanceReferences.length === 0
    ) {
      return;
    }
    ...
    if (objectType === this.rhino.ObjectType.Mesh) {
      ...
      if (this.instanceIdToDefinition.has(parentDefinitionId)) {
        let instanceDefinition =
          this.instanceIdToDefinition.get(parentDefinitionId);
  ...
  ImportRhinoMesh(rhinoDoc, rhinoMesh, rhinoObject, rhinoInstanceReferences) {
    ...
    let materialIndex = this.GetMaterialIndex(
      rhinoDoc,
      rhinoObject,
      rhinoInstanceReferences
    );
    ...
    if (userStrings.length > 0) {
      ...
      for (let i = 0; i < userStrings.length; i++) {
        ...
        propertyGroup.AddProperty(
          new Property(PropertyType.Text, userString[0], userString[1])
        );
      }
    ...
    if (rhinoInstanceReferences.length !== 0) {
      ...
      for (let i = rhinoInstanceReferences.length - 1; i >= 0; i--) {
        ...
        let rhinoInstanceReferenceMatrix =
          rhinoInstanceReferenceGeometry.xform.toFloatArray(false);
  ...
  GetMaterialIndex(rhinoDoc, rhinoObject, rhinoInstanceReferences) {
    function GetRhinoMaterial(rhino, rhinoObject, rhinoInstanceReferences) {
      ...
      if (
        ...
        rhinoAttributes.materialSource ===
        rhino.ObjectMaterialSource.MaterialFromParent
      ) {
        if (rhinoInstanceReferences.length !== 0) {
    ...
    function ConvertRhinoMaterial(rhinoMaterial, callbacks) {
      ...
      function IsWhite(rhinoColor) {
        return (
          rhinoColor.r === 255 && rhinoColor.g === 255 && rhinoColor.b === 255
        );
      }
    ...
    let rhinoMaterial = GetRhinoMaterial(
      this.rhino,
      rhinoObject,
      rhinoInstanceReferences
    );
