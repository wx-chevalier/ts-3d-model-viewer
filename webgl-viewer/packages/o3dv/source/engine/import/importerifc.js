...
export class ImporterIfc extends ImporterBase {
  ...
  ImportContent(fileContent, onFinish) {
    if (this.ifc === null) {
      LoadExternalLibrary("loaders/web-ifc-api-browser.js")
        .then(() => {
  ...
  ImportIfcMesh(modelID, ifcMesh) {
    ...
    for (
      let geometryIndex = 0;
      geometryIndex < ifcGeometries.size();
      geometryIndex++
    ) {
      const ifcGeometry = ifcGeometries.get(geometryIndex);
      const ifcGeometryData = this.ifc.GetGeometry(
        modelID,
        ifcGeometry.geometryExpressID
      );
      const ifcVertices = this.ifc.GetVertexArray(
        ifcGeometryData.GetVertexData(),
        ifcGeometryData.GetVertexDataSize()
      );
      const ifcIndices = this.ifc.GetIndexArray(
  ...
  ImportProperties(modelID) {
    const lines = this.ifc.GetLineIDsWithType(
      modelID,
      ...
      rel.RelatedObjects.forEach((objectRelID) => {
        ...
        propSet.HasProperties.forEach((property) => {
          ...
          switch (property.NominalValue.label) {
            ...
            case "IFCLABEL":
            case "IFCIDENTIFIER":
              elemProperty = new Property(
                PropertyType.Text,
                propertyName,
                this.GetIFCString(property.NominalValue.value)
              );
              break;
            case "IFCBOOLEAN":
            case "IFCLOGICAL":
