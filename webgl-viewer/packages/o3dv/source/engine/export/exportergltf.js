...
export class ExporterGltf extends ExporterBase {
  ...
  CanExport(format, extension) {
    return (
      (format === FileFormat.Text && extension === "gltf") ||
      (format === FileFormat.Binary && extension === "glb")
    );
  }
  ...
  ExportBinaryContent(exporterModel, files) {
    ...
    let glbSize =
      12 + 8 + mainJsonBufferAlignedLength + 8 + mainBinaryBufferAlignedLength;
    ...
    WriteCharacters(
      glbWriter,
      32,
      mainJsonBufferAlignedLength - mainJsonBufferLength
    );
    ...
    WriteCharacters(
      glbWriter,
      0,
      mainBinaryBufferAlignedLength - mainBinaryBufferLength
    );
  ...
  GetMainBuffer(meshDataArr) {
    ...
    for (let meshIndex = 0; meshIndex < meshDataArr.length; meshIndex++) {
      ...
      mainBufferSize += meshData.buffer.GetByteLength(
    ...
    for (let meshIndex = 0; meshIndex < meshDataArr.length; meshIndex++) {
      ...
      for (
        ...
        primitiveIndex < meshData.buffer.PrimitiveCount();
        primitiveIndex++
      ) {
  ...
  GetMainJson(meshDataArr) {
    ...
    for (let meshIndex = 0; meshIndex < meshDataArr.length; meshIndex++) {
      ...
      for (
        ...
        let bufferViewCreator = new BufferViewCreator(
          mainJson,
          meshData.offsets[primitiveIndex]
        );
        let indicesBufferView = bufferViewCreator.AddBufferView(
          primitive.indices.length * this.components.index.size
        );
        let verticesBufferView = bufferViewCreator.AddBufferView(
          primitive.vertices.length * this.components.number.size
        );
        let colorsBufferView = null;
        if (primitive.colors.length > 0) {
          colorsBufferView = bufferViewCreator.AddBufferView(
            primitive.colors.length * this.components.number.size
          );
        }
        let normalsBufferView = bufferViewCreator.AddBufferView(
          primitive.normals.length * this.components.number.size
        );
        let uvsBufferView = null;
        if (primitive.uvs.length > 0) {
          uvsBufferView = bufferViewCreator.AddBufferView(
            primitive.uvs.length * this.components.number.size
          );
        }
  ...
  ExportMaterials(exporterModel, mainJson, addTexture) {
    function ExportMaterial(obj, mainJson, material, addTexture) {
      ...
      let baseColorTexture = GetTextureParams(
        mainJson,
        material.diffuseMap,
        addTexture
      );
      if (baseColorTexture !== null) {
        if (!material.multiplyDiffuseMap) {
          jsonMaterial.pbrMetallicRoughness.baseColorFactor = ColorToRGBA(
      ...
      if (material.type === MaterialType.Physical) {
        let metallicTexture = GetTextureParams(
          mainJson,
          material.metalnessMap,
          addTexture
        );
        if (metallicTexture !== null) {
          jsonMaterial.pbrMetallicRoughness.metallicRoughnessTexture =
            metallicTexture;
        } else {
          jsonMaterial.pbrMetallicRoughness.metallicFactor = material.metalness;
          jsonMaterial.pbrMetallicRoughness.roughnessFactor =
            material.roughness;
        }
      }
      let normalTexture = GetTextureParams(
        mainJson,
        material.normalMap,
        addTexture
      );
