...
export class TouchInteraction {
  ...
  GetPositionFromEvent(canvas, ev) {
    ...
    if (ev.touches.length !== 0) {
      ...
      coord = GetDomElementClientCoordinates(
        canvas,
        touchEv.pageX,
        touchEv.pageY
      );
...
export class ClickDetector {
  ...
  Move(currentPosition) {
    ...
    if (this.startPosition !== null) {
      ...
      const currentDistance = CoordDistance2D(
        this.startPosition,
        currentPosition
      );
...
export class Navigation {
  ...
  GetFitToSphereCamera(center, radius, fov) {
    ...
    let centerEyeDirection = SubCoord3D(
      fitCamera.eye,
      fitCamera.center
    ).Normalize();
  ...
  OnMouseMove(ev) {
    ...
    if (this.onMouseMove) {
      let mouseCoords = GetDomElementClientCoordinates(
        this.canvas,
        ev.clientX,
        ev.clientY
      );
    ...
    if (navigationType === NavigationType.Orbit) {
      ...
      let eyeCenterDistance = CoordDistance3D(
        this.camera.eye,
        this.camera.center
      );
  ...
  OnTouchMove(ev) {
    ...
    if (navigationType === NavigationType.Orbit) {
      ...
      let panRatio =
        0.001 * CoordDistance3D(this.camera.eye, this.camera.center);
      this.Pan(moveDiff.x * panRatio, moveDiff.y * panRatio);
    }
  ...
  Orbit(angleX, angleY) {
    ...
    let viewDirection = SubCoord3D(
      this.camera.center,
      this.camera.eye
    ).Normalize();
    let horizontalDirection = CrossVector3D(
      viewDirection,
      this.camera.up
    ).Normalize();
    ...
    if (this.fixUpVector) {
      ...
      let verticalDirection = CrossVector3D(
        horizontalDirection,
        viewDirection
      ).Normalize();
      this.camera.eye.Rotate(
  ...
  Context(clientX, clientY) {
    if (this.onContext) {
      ...
      let localCoords = GetDomElementClientCoordinates(
        this.canvas,
        clientX,
        clientY
      );
