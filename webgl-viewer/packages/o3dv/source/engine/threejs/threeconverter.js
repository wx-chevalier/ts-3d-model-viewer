...
export function ConvertModelToThreeObject(model, params, output, callbacks) {
  function CreateThreeMaterial(
    ...
    function LoadTexture(
      ...
      if (texture.mimeType !== null) {
        textureObjectUrl = CreateObjectUrlWithMimeType(
          texture.buffer,
          texture.mimeType
        );
      } else {
  ...
  function CreateThreeMesh(model, meshInstanceId, modelThreeMaterials) {
    ...
    for (let i = 0; i < triangleIndices.length; i++) {
      ...
      if (!modelToThreeMaterials.has(modelMaterialIndex)) {
        modelToThreeMaterials.set(
          modelMaterialIndex,
          meshThreeMaterials.length
        );
    ...
    for (let i = 0; i < groups.length; i++) {
      let group = groups[i];
      threeGeometry.addGroup(
        group.start * 3,
        (group.end - group.start + 1) * 3,
        i
      );
    }
  ...
  function ConvertNodeHierarchy(
    ...
    RunTasksBatch(meshInstances.length, 100, {
      runTask: (firstMeshInstanceIndex, lastMeshInstanceIndex, onReady) => {
        for (
          let meshInstanceIndex = firstMeshInstanceIndex;
          ...
          let meshInstanceId = new MeshInstanceId(
            node.GetId(),
  ...
  for (
    ...
    let threeMaterial = CreateThreeMaterial(
      stateHandler,
      model,
      materialIndex,
      shadingType,
