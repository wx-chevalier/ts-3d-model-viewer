...
export function GenerateCone(
  ...
  if (isZeroBottom) {
    ...
    for (let i = 0; i < segments; i++) {
      ...
      bottomPolygon.push(
        generator.AddVertex(bottomVertex.x, bottomVertex.y, 0.0)
      );
    }
  }
...
export function GenerateSphere(genParams, radius, segments, smooth) {
  ...
  for (let levelIndex = 1; levelIndex < levels - 1; levelIndex++) {
    ...
    for (
      let cylindricalIndex = 0;
      cylindricalIndex < segments;
      cylindricalIndex++
    ) {
    ...
    if (levelIndex > 1) {
      helper.GenerateSurfaceBetweenPolygons(
        allLevelVertices[allLevelVertices.length - 1],
        levelVertices
      );
    }
