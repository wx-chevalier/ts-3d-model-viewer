...
export class MeasureTool {
  ...
  Click(mouseCoordinates) {
    let intersection =
      this.viewer.GetMeshIntersectionUnderMouse(mouseCoordinates);
  ...
  AddMarker(intersection) {
    ...
    if (this.markers.length === 2) {
      ...
      this.viewer.AddExtraObject(
        CreateLineFromPoints([aPoint, bPoint], material)
      );
    }
  }
    ...
    if (this.markers.length === 0) {
      ...
      if (calcResult.pointsDistance !== null) {
        AddValue(
          ...
          "measure_distance",
          "Distance of points",
          calcResult.pointsDistance.toFixed(3)
        );
      }
      if (calcResult.parallelFacesDistance !== null) {
        AddValue(
          this.panel,
          "measure_distance_parallel",
          "Distance of parallel faces",
          calcResult.parallelFacesDistance.toFixed(3)
        );
      }
