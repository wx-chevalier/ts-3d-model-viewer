...
} from "../engine/model/modelutils.js";
...
export class Website {
  constructor(parameters) {
    ...
    this.navigator = new Navigator(
      this.parameters.navigatorDiv,
      this.parameters.navigatorSplitterDiv
    );
  ...
  Load() {
    ...
    HandleEvent(
      "theme_on_load",
      this.settings.themeId === Theme.Light ? "light" : "dark"
    );
  ...
  OnModelClicked(button, mouseCoordinates) {
    ...
    if (meshUserData === null) {
      ...
      this.navigator.SetSelection(
        new Selection(SelectionType.Mesh, meshUserData.originalMeshId)
      );
    }
  }
  ...
  OnModelContextMenu(globalMouseCoordinates, mouseCoordinates) {
    ...
    if (meshUserData === null) {
      ...
      if (this.navigator.MeshItemCount() > 1) {
        let isMeshIsolated = this.navigator.IsMeshIsolated(
          meshUserData.originalMeshId
        );
        items.push({
    ...
    ShowListPopup(items, {
      calculatePosition: (contentDiv) => {
        return CalculatePopupPositionToScreen(
  ...
  UpdateMeshesSelection() {
    ...
    this.viewer.SetMeshesHighlight(this.highlightColor, (meshUserData) => {
      if (
        selectedMeshId !== null &&
        meshUserData.originalMeshId.IsEqual(selectedMeshId)
      ) {
        return true;
      }
      return false;
  ...
  UpdateEnvironmentMap() {
    ...
    this.viewer.SetEnvironmentMapSettings(
      envMapTextures,
  ...
  InitToolbar() {
    ...
    function AddPushButton(
      ...
      let button = toolbar.AddImagePushButton(
        imageName,
        imageTitle,
        false,
        (isSelected) => {
    ...
    function AddRadioButton(
      ...
      let buttons = toolbar.AddImageRadioButton(
        imageData,
        selectedIndex,
        (buttonIndex) => {
          onClick(buttonIndex);
    ...
    AddRadioButton(
      this.toolbar,
      ["fix_up_on", "fix_up_off"],
      ["Fixed up vector", "Free orbit"],
      0,
      ["only_on_model"],
      (buttonIndex) => {
        if (buttonIndex === 0) {
    ...
    let measureToolButton = AddPushButton(
      this.toolbar,
      "measure",
      "Measure",
      ["only_full_width", "only_on_model"],
      (isSelected) => {
  ...
  InitSidebar() {
    this.sidebar.Init({
      ...
      onEdgeDisplayChange: () => {
        HandleEvent(
          "edge_display_changed",
          this.settings.showEdges ? "on" : "off"
        );
  ...
  InitNavigator() {
    ...
    function GetMeshesForMaterial(viewer, model, materialIndex) {
      ...
      viewer.EnumerateMeshesUserData((meshUserData) => {
        if (
          materialIndex === null ||
          meshUserData.originalMaterials.indexOf(materialIndex) !== -1
        ) {
    ...
    this.navigator.Init({
      ...
      onMaterialSelected: (materialIndex) => {
        this.sidebar.AddMaterialProperties(
          this.model.GetMaterial(materialIndex)
        );
      },
      onResize: () => {
        this.Resize();
  ...
  InitCookieConsent() {
    ...
    let acceptButton = AddDiv(
      popupDiv,
      "ov_button ov_floating_panel_button",
      "Accept"
    );
